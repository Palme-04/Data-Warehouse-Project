...
BEGIN
    ...
    FROM (
        SELECT *,
        ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) AS newest_entry
        FROM bronze.crm_cust_info
        WHERE cst_id IS NOT NULL
    )t WHERE newest_entry = 1;
    ...
    SELECT 
        ...
        CASE WHEN sls_sales IS NULL OR sls_sales <= 0 or sls_sales != sls_quantity * ABS(sls_price) 
            ...
            ELSE sls_sales
        END AS sls_sales,
        CASE WHEN sls_quantity IS NULL THEN 0 ELSE sls_quantity
        END AS sls_quantity,
    ...
    PRINT '-----------------------------------'
		...
		PRINT '## Total load time for silver layer: ' + CAST (DATEDIFF(second, @total_start_time, @total_end_time) AS NVARCHAR) + ' sec';

    END TRY
    BEGIN CATCH
		PRINT '========================';
		PRINT 'ERROR OCCURED IN LOAD OF SILVER LAYER';
